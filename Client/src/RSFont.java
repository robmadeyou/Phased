...
public final class RSFont extends DrawingArea {
	...
	public RSFont(boolean flag, String s, NamedArchive archive) {
	...
	public void method385(int i, String s, int j, int l) {
		...
		for(int i1 = 0; i1 < s.length(); i1++) {
			char c = s.charAt(i1);
			if(c != ' ')
				method392(fontPixels[c], l + characterDrawXOffsets[c], j + characterDrawYOffsets[c], characterWidths[c], characterHeights[c], i);
	...
	public void method386(int i, String s, int j, int k, int l) {
		...
		for(int i1 = 0; i1 < s.length(); i1++) {
			char c = s.charAt(i1);
			if(c != ' ')
				method392(fontPixels[c], j + characterDrawXOffsets[c], l + characterDrawYOffsets[c] + (int)(Math.sin((double)i1 / 2D + (double)k / 5D) * 5D), characterWidths[c], characterHeights[c], i);
			j += characterScreenWidths[c];
		}
	}
	...
	public void method387(int i, String s, int j, int k, int l) {
		...
		for(int i1 = 0; i1 < s.length(); i1++) {
			char c = s.charAt(i1);
			if(c != ' ')
				method392(fontPixels[c], i + characterDrawXOffsets[c] + (int)(Math.sin((double)i1 / 5D + (double)j / 5D) * 5D), k + characterDrawYOffsets[c] + (int)(Math.sin((double)i1 / 3D + (double)j / 5D) * 5D), characterWidths[c], characterHeights[c], l);
			i += characterScreenWidths[c];
		}
	}
	...
	public void method388(int i, String s, int j, int k, int l, int i1) {
		...
		for(int k1 = 0; k1 < s.length(); k1++) {
			char c = s.charAt(k1);
			if(c != ' ')
				method392(fontPixels[c], l + characterDrawXOffsets[c], k + characterDrawYOffsets[c] + (int)(Math.sin((double)k1 / 1.5D + (double)j) * d), characterWidths[c], characterHeights[c], i1);
			l += characterScreenWidths[c];
		}
	}
	...
	public void method389(boolean flag1, int i, int j, String s, int k) {
		...
		for(int i1 = 0; i1 < s.length(); i1++)
			if(s.charAt(i1) == '@' && i1 + 4 < s.length() && s.charAt(i1 + 4) == '@') {
				...
				if(c != ' ') {
					if(flag1)
					method392(fontPixels[c], i + characterDrawXOffsets[c] + 1, k + characterDrawYOffsets[c] + 1, characterWidths[c], characterHeights[c], 0);
					method392(fontPixels[c], i + characterDrawXOffsets[c], k + characterDrawYOffsets[c], characterWidths[c], characterHeights[c], j);
				}
				i += characterScreenWidths[c];
			}
		if(aBoolean1499)
	...
	public void method390(int i, int j, String s, int k, int i1) {
		...
		for(int k1 = 0; k1 < s.length(); k1++)
			if(s.charAt(k1) == '@' && k1 + 4 < s.length() && s.charAt(k1 + 4) == '@') {
				...
				char c = s.charAt(k1);
				if(c != ' ') {
					method394(192, i + characterDrawXOffsets[c] + 1, fontPixels[c], characterWidths[c], i1 + characterDrawYOffsets[c] + 1, characterHeights[c], 0);
					method394(j1, i + characterDrawXOffsets[c], fontPixels[c], characterWidths[c], i1 + characterDrawYOffsets[c], characterHeights[c], j);
				}
				i += characterScreenWidths[c];
