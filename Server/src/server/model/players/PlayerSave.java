				...
				case 2:
                  ...
                  p.muteEnd = Long.parseLong(token2);
                     ...
                     p.banEnd = Long.parseLong(token2);
						...
						p.pkPoints = Integer.parseInt(token2);	
					} else if (token.equals("level-Points")) {
                                                p.levelPoints = Integer.parseInt(token2);
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("loyalty-title = ", 0, 15);
			characterfile.write(Integer.toString(p.loyaltyTitle), 0, Integer.toString(p.loyaltyTitle).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("level-Points = ", 0, 15);
            characterfile.write(Integer.toString(p.levelPoints), 0, Integer.toString(p.levelPoints).length());
			characterfile.newLine();
