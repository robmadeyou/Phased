...
if(c.playerEquipment[c.playerWeapon] == 13907) {
c.vSpearLeft -=1;
c.degradeVSpear();
}
if(c.playerEquipment[c.playerChest] == 13860) {
c.zTopLeft -=1;
c.degradeZTop();
}
if(c.playerEquipment[c.playerLegs] == 13863) {
c.zBottomLeft -=1;
c.degradeZBottom();
}
if(c.playerEquipment[c.playerWeapon] == 13904){
c.statLeft -= 1;
c.degradeStat();
}
if(c.playerEquipment[c.playerChest] == 13889) {
c.vTopLeft -=1;
c.degradeVTop();
}
if(c.playerEquipment[c.playerLegs] == 13895) {
c.vLegsLeft -=1;
c.degradeVLegs();
}
if(c.playerEquipment[c.playerWeapon] == 13869) {
c.zStaffLeft -=1;
c.degradeZStaff();
}
if(c.playerEquipment[c.playerHat] == 13866) {
c.zHoodLeft -=1;
c.degradeZHood();
}
if(c.playerEquipment[c.playerChest] == 13872) {
c.mBodyLeft -=1;
c.degradeMBody();
}
if(c.playerEquipment[c.playerLegs] == 13875) {
c.mChapsLeft -=1;
c.degradeMChaps();
}
if(c.playerEquipment[c.playerChest] == 13886) {
c.sTopLeft -=1;
c.degradeSTop();
}
if(c.playerEquipment[c.playerLegs] == 13892) {
c.sLegsLeft -=1;
c.degradeSLegs();
}
if(c.playerEquipment[c.playerHat] == 13898) {
c.sHelmLeft -=1;
c.degradeSHelm();
}
			}
	...
	**/
		...
		Client o = (Client)Server.playerHandler.players[i];
		  ...
		  if (!c.autocasting) {
            ...
            for (int u : c.Bolts)  {
                for (int y : c.BOWS)  {
                    if(y == c.playerEquipment[c.playerWeapon] && u == c.playerEquipment[c.playerArrows] && c.playerEquipment[c.playerWeapon] != 4734 && c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 18357){
                        c.sendMessage("You cant use arrows with this bow.");
                        return;
                    }
                }
            }
		...
		if (Server.playerHandler.players[i] != null) {
			...
			if(c.attackTimer <= 0) {
				...
				if(correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && usingBow && usingCross && !c.usingMagic) {
					c.sendMessage("You can't use "+c.getItems().getItemName(c.playerEquipment[c.playerArrows]).toLowerCase()+"s with a "+c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()+".");
					c.stopMovement();
					resetPlayerAttack();
					return;
	...
	if(!usingCross && !usingArrows && usingBow && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223)) {
				...
				if(failed < 10) {
					...
					return;
							...
							c.startAnimation(12575);
							c.getItems().updateSpecialBar();
							c.sendMessage("You have leeched " +Misc.optimizeText(c2.playerName)+"'s special attack!");
	...
	public void playerDelayedHit(int i) {
		if (Server.playerHandler.players[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.lastWeaponUsed == 11235 || c.bowSpecShot == 1) {
					if (Misc.random(o.getCombat().calculateRangeDefence()) > Misc.random(calculateRangeAttack()))
						damage2 = 0;
				}
		...
		case 13346: // Virtus Legs
			damage *= .04;
				...
				Server.playerHandler.players[i].singleCombatDelay = System.currentTimeMillis();
				Server.playerHandler.players[i].killerId = c.playerId;
				Server.playerHandler.players[i].CIcon = 1;
...
if (damage2 != -1 && rangeDouble[j] == c.playerEquipment[c.playerWeapon] || c.bowSpecShot == 1) {
...
Server.playerHandler.players[i].handleHitMask(damage2);
				...
				if(c.MAGIC_SPELLS[c.oldSpellId][6] != 0) {
					...
					if (!c.magicFailed) {
						...
						//Server.playerHandler.players[i].setHitUpdateRequired(true);
						Server.playerHandler.players[i].handleHitMask(damage);
						Server.playerHandler.players[i].CIcon = 2;
					}
				}
				applySmite(i, damage);
				c.killedBy = Server.playerHandler.players[i].playerId;	
...
public void appendMultiBarrage(int playerId, boolean splashed) {
	...
	public void applyPlayerClawDamage(int i, int damageMask, int damage){
		...
		switch(damageMask) {
			...
			//Server.playerHandler.players[i].playerLevel[3] -= damage;
			Server.playerHandler.players[i].CIcon = 0;
			Server.playerHandler.players[i].handleHitMask(damage);
			Server.playerHandler.players[i].dealDamage(damage);
			Server.playerHandler.players[i].damageTaken[c.playerId] += damage;
			c.totalPlayerDamageDealt += damage;
...
break;
	...
	**/
	...
	public boolean checkReqs() {
		...
		if(!Server.playerHandler.players[c.playerIndex].inWild() && !Server.playerHandler.players[c.playerIndex].InDung() && !Server.playerHandler.players[c.playerIndex].inDuelArena() && Server.playerHandler.players[c.playerIndex].safeTimer <= 0) {
	...
	**/
	...
	public int rangeMaxHit() {
	    ...
	    rangehit += c.playerLevel[4] / 17;
			...
			rangehit = 4;
            rangehit += c.playerLevel[4] / 5;
        } else if (Arrows == 15243 && weapon == 15241) {//hand cannon
			rangehit = 6;
            rangehit += c.playerLevel[4] / 5;
...
if (c.playerEquipment[c.playerWeapon] == 15241)
	...
	**/
	...
	public int mageAtk()
    {
		...
		if(c.curseActive[12])  // Leech Magic
			attackLevel += c.getLevelForXP(c.playerXP[6]) * 0.1;
        if (c.prayerActive[4]) 
			attackLevel *= 1.05;
...
try {
	...
	public void Zammybook() {
		...
		public void handleDfsNPC() {
			...
			if (System.currentTimeMillis() - c.dfsDelay > 30000) {
			if (c.npcIndex > 0 && Server.npcHandler.npcs[c.npcIndex] != null) {
				final int damage = Misc.random(35) + 5;
				c.startAnimation(6696);
				c.gfx0(1165);
				Server.npcHandler.npcs[c.npcIndex].hitUpdateRequired2 = true;
				Server.npcHandler.npcs[c.npcIndex].updateRequired = true;
...
return;
	...
	public static void addCharge(Client c) {
		...
		if(c.dfsCount >= 40) {
			c.dfsCount = 40;
			c.sendMessage("Your Dragonfire Shield is fully Charged.");
			return;
		}
	}
}	